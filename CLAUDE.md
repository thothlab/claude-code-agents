...
**Выбор scope при сохранении (ошибка по умолчанию — всё писать как `project:`):**
- Думай: «Будет ли это полезно в следующем Android/KMP/Web-проекте?» Если да → `cross-project / <стек>`, даже если паттерн обнаружен в конкретном проекте.
- `project:<name>` — только если содержит специфику этого проекта: имена классов, пути, токены, бизнес-логику.
- Предпочтения пользователя → `universal`.
- Обобщаемые архитектурные решения, UX-паттерны, debugging-приёмы → `cross-project`.

**Когда вспоминать (`/agentmemory:recall`):**
- **В начале каждой задачи** — quick recall по ключевым словам темы (имя фичи, технология, тип бага). Цель — не пропустить релевантный generalized-урок из других проектов.
- Также: когда пользователь говорит «вспомни / на чём остановились / что мы делали».

**Что делать после recall (обязательный алгоритм — не ждать указаний пользователя):**
1. Для топ-результатов — получить полный content через `expandIds` в `memory_smart_search`.
2. Отфильтровать по scope (правила ниже).
3. Из прошедших фильтр: кратко сообщить «Нашёл N записей: [1 предложение на каждую]» и **сразу применить** контекст к текущей задаче — не просить пользователя объяснять что с этим делать.
4. Если ничего не прошло фильтр — одна строка: «Релевантных записей не найдено.» Не elaborate.
